...
#     "shapely>=2.1",
#     "geoarrow-rust-io>=0.6",
# ]
# ///
"""BUILDINGS ARE PART OF THE SURFACE, NOT A LAYER ON TOP OF IT.

This is `xsql-naip-tiles.py` with Overture building footprints folded into the height
field. A footprint is rasterised onto the same tile lattice everything else lives on, and
the texels inside it are RAISED TO A FLAT ROOF at `ground + height`. So the mesh carries
the buildings, the NAIP photograph drapes over the roofs (NAIP is an orthophoto, so the
roof pixels are already the roof), and the hillshade lights the walls, all without a
second layer and without deck knowing that buildings exist.

The alternative, an extruded `PolygonLayer` flying above the terrain, is the obvious
implementation and it is a worse one HERE: it needs a per-building base elevation sampled
off the same height field anyway, it z-fights the surface where the two disagree, and it
cannot take the photograph. What it does buy is per-building picking and attributes, which
this does not have. Both are legitimate; this notebook is the first one.

WHAT THE FOOTPRINTS ARE READ FROM. Overture Maps on `overturemaps-us-west-2`, release
pinned in `overture_core.py`, streamed with the same obstore + GeoParquet machinery the
COGs use. Two things about that read are worth knowing:

  * THE FILE INDEX IS THE WHOLE TRICK. A theme is ~512 parquet files of half a gigabyte
    and there is no catalog, so the first run of a theme reads 512 GeoParquet footers for
    their bboxes (~100 s) and caches the result in `.cache/`. After that an AOI touches
    only the file (usually one) that overlaps it: ~1.4 s. Measured against the two
    alternatives: a DuckDB `read_parquet` scan with a bbox predicate is 35 s EVERY time,
    and `GeoParquetDataset.open` refuses these files outright, because the geometry column
    is Polygon in some parts and MultiPolygon in others.
  * HEIGHT IS OFTEN MISSING AND THAT IS NOT AN ERROR. Overture carries `height` where a
    source had one and ML-estimated it for much of the rest. Measured: 72% of 9,837
    buildings in Sedona AZ, 58% of 3,102 in downtown Salt Lake. The rest fall back to
    `num_floors * 3 m` and then to the `Default height` control, and the fetch cell prints
    the split so a scene is never quietly half invented.

THE PART THAT NEEDED CARE: A ROOF IS FLAT AND A HILLSIDE IS NOT. Adding `height` to
whatever the terrain does under a footprint gives you a building with a sloped roof
sitting in a hole. So each footprint gets ONE base elevation, sampled at its centroid from
the tile that owns the centroid, and every texel inside it is set to `base + height` by
`maximum`. One base per building rather than one per tile is also what keeps a footprint
straddling a tile boundary from stepping halfway across: both tiles compute the roof from
the same number.

RESOLUTION IS WHAT DECIDES WHETHER YOU SEE THEM. The mesh samples the height field every
`Texels / quad` texels, so a building narrower than one quad may land between vertices and
never reach the geometry. At the default 1.5 m detail and 4 texels/quad a quad is 6 m,
which is a house. The buildings cell measures the median footprint against the quad size
and says so. `Texels / quad = 1` and `Detail` at 1 m or finer is the setting that draws
suburbs; the coarse end draws downtown only.

Everything below this is `xsql-naip-tiles.py` unchanged, and the notes it carries about
the tiling still hold:

THE INVARIANT: **no array here is ever AOI-sized.** Every array is tile-sized. A tile is a
fixed number of texels at a fixed metres-per-texel, the DEM and NAIP are read PER TILE
WINDOW straight out of the COGs, and the AOI decides only how many tiles exist. Ground
resolution is constant. A wider box costs more tiles; it never costs sharpness.
...
Buildings are in the height field, so they colour and shade with whichever one is showing.
...
Run:  uv run marimo edit xsql-naip-buildings.py

`naip.py` and `overture_core.py` must sit next to this file.
"""
...
def _():
    ...
    import naip
    import overture_core as overture
...
def _(mo):
    mo.md("""
    # Buildings in the surface

    Overture building footprints are rasterised onto the same tile lattice as everything
    else and **raised to a flat roof at `ground + height`**, so the mesh carries the
    buildings, the NAIP orthophoto drapes over the roofs and the hillshade lights the
    walls. No second layer, and nothing in deck knows a building exists.

    Height comes from Overture where it has one and from `num_floors * 3 m` where it does
    not; the rest fall back to a control you can set to zero. The fetch cell prints the
    split, so a scene is never quietly half invented.

    Resolution still comes from **Detail** and **Tile texels**, never from the box, and it
    is what decides whether a house survives to the geometry: **Texels / quad = 1** puts a
    vertex on every texel, which is what a wall needs.
...
def _(mo):
    ...
    detail = mo.ui.dropdown(
        ...
        # 4 m AND CLIMB, rather than starting fine. Buildings want resolution (a footprint
        # has to land on several lattice samples to become a roof, and the buildings cell
        # counts the ones that did not), but the cost is quadratic in BOTH directions and
        # the failure at the top is silent in the browser: at 1 m/texel with a vertex per
        # texel a 5 x 4.5 km box is 606 MB of positions and 100M triangles, deck fails to
        # allocate, and what you get is the basemap with no mesh on it and no error
        # anywhere. So this starts where a town-sized box certainly renders and the mesh
        # cell prints the bill each time you step down. Measured on the Sedona box:
        #   4 m, 2 texels/quad -> 15 tiles, 12 MB positions, 2.0M triangles
        #   2 m, 2 texels/quad -> 63 tiles, 50 MB, 8.3M
        #   1 m, 1 texel/quad  -> 192 tiles, 606 MB, 100.7M  (does not draw)
        value="4 m / texel",
        label="Detail",
    )
...
@app.cell
def _(mo):
    # Sedona, AZ: a town of low-rise buildings inside red rock canyons, which is the scene
    # that argues for putting buildings in the surface rather than over it. ~5 x 4 km.
    # Overture has a height for 72% of the 9,837 footprints here, median 4.7 m.
    #
    # Others worth pasting in, buildings first:
    #   Downtown Salt Lake, UT    [-111.91, 40.75, -111.86, 40.78]   towers, 58% with height
    #   Flagstaff + Mt Elden, AZ  [-111.72, 35.15, -111.55, 35.27]
    #   Big + Little Cottonwood   [-111.79, 40.55, -111.63, 40.66]   the tiles default
    ...
    get_bbox, set_bbox = mo.state([-111.80, 34.845, -111.745, 34.885])
...
def _(
    ...
    picker = Map(
        ...
        view_state={"longitude": -111.77, "latitude": 34.865, "zoom": 12.5, "pitch": 0},
...
@app.cell
def _(mo):
    # THE BUILDING CONTROLS, and they are up here rather than in the scene row at the bottom
    # because two of them decide what gets FETCHED and how the height field is built, which
    # is the same reason `Detail` sits above the map. Only `Buildings` reaches the fetch: the
    # other two are read when footprints are stamped into the height field, so changing a
    # fallback height re-stamps without re-reading a single parquet file.
    buildings_on = mo.ui.switch(value=True, label="Buildings")
    # 0 MEANS SKIP, which is the honest option and the reason this is a number rather than a
    # switch. Overture has no height for roughly a quarter to a half of its footprints
    # depending on where you are, and there are two defensible answers: draw them at a
    # nominal height so the town has the right shape, or draw only what is measured. The
    # fetch cell prints the split either way, so the scene never lies about which it is.
    default_height = mo.ui.number(
        start=0.0, stop=60.0, step=0.5, value=5.0, debounce=True,
        label="Default height (m, 0 = skip)",
    )
    building_scale = mo.ui.number(
        start=0.0, stop=5.0, step=0.1, value=1.0, debounce=True, label="Building height x"
    )
    mo.vstack(
        [
            mo.hstack(
                [buildings_on, default_height, building_scale], justify="start", gap=2
            ),
            mo.md(
                "<small>Footprints are **raised into the surface itself**, not drawn as a "
                "layer over it, so the photograph drapes the roofs and the hillshade lights "
                "the walls. Building height is exaggerated by **Elevation scale** along "
                "with the terrain; this multiplier is on top of that.</small>"
            ),
        ],
        gap=0.5,
    )
    return building_scale, buildings_on, default_height


@app.cell
async def _(CACHE, bbox, buildings_on, get_started, mo, np, overture, shapely):
    # THE FOOTPRINTS. Overture buildings for the AOI, streamed from the public
    # `overturemaps-us-west-2` bucket with the same obstore machinery the COGs use.
    #
    # WHY THERE IS AN INDEX AND WHY IT IS CACHED. A theme is ~512 parquet files of about
    # half a gigabyte and Overture publishes no catalog of what is in which file. The
    # file-level bbox is in each GeoParquet footer, so the index is 512 footer reads, ~100 s,
    # done once per release per theme and written to `.cache/`. Afterwards an AOI reads only
    # the file that overlaps it, measured at 1.4 s for 9,850 buildings.
    #
    # The two alternatives, both tried: `GeoParquetDataset.open` REFUSES these files, because
    # the geometry column is Polygon in some parts and MultiPolygon in others and a dataset
    # wants one type; and DuckDB `read_parquet` with a bbox predicate works but re-reads the
    # footers on every query, measured at 35 s each time.
    #
    # ONE THEME TODAY, THE MACHINERY IS PLURAL. `overture.THEMES` names every theme/type pair
    # and the index is per path, so places, transportation and land cover come in through
    # exactly this cell with one more entry in the list. Only buildings carry a height, so
    # only buildings are read here.
    if not get_started() or not buildings_on.value:
        b_poly, b_height, b_bounds, b_measured = None, None, None, 0
        if get_started():
            print("buildings off: the surface is bare ground")
    else:
        _store = overture.get_store()
        if not overture.index_path("buildings", str(CACHE)).exists():
            print(
                f"building the Overture file index for release "
                f"{overture.OVERTURE_RELEASE} (512 GeoParquet footers, ~100 s). It is "
                f"cached in {CACHE}/ and every AOI after this one costs about a second."
            )
        _index = await overture.file_index(_store, "buildings", cache_dir=str(CACHE))
        _hits = overture.index_hits(_index, bbox)
        _table = await overture.load_wkb(
            _store,
            "buildings",
            tuple(bbox),
            _index,
            columns=["height", "num_floors", "is_underground", "class"],
        )
        if _table is None or _table.num_rows == 0:
            b_poly, b_height, b_bounds, b_measured = None, None, None, 0
            print(f"Overture: no buildings in this AOI ({len(_hits)} file(s) searched)")
        else:
            _h = np.asarray(_table.column("height").to_pylist(), dtype="float64")
            # to_pylist, not iteration: iterating an Arrow column yields Arrow SCALARS, and
            # a null one is not `None` (it is a NullScalar, which `float()` refuses with a
            # TypeError about __index__). Python objects are what the numpy conversions want.
            _fl = np.asarray(
                [np.nan if v is None else float(v) for v in _table.column("num_floors").to_pylist()],
                dtype="float64",
            )
            _under = np.asarray(
                [bool(v) for v in _table.column("is_underground").to_pylist()], dtype=bool
            )
            b_measured = int(np.isfinite(_h).sum())
            # THE HEIGHT LADDER, most trustworthy first. `height` is a real measurement or
            # Overture's ML estimate; `num_floors * 3 m` is a convention and is only
            # reachable for the few percent that have floors but no height; the rest fall to
            # the control, which may be 0, meaning "do not invent this one".
            b_height = np.where(np.isfinite(_h), _h, np.nan)
            _from_floors = np.isnan(b_height) & np.isfinite(_fl)
            b_height = np.where(_from_floors, _fl * 3.0, b_height)

            b_poly = shapely.from_wkb(np.asarray(_table.column("geometry")))
            # Underground structures are in the theme and have real heights; stamping them
            # into a terrain surface would put a subway box on a hillside. A NaN height is
            # kept (the Default height control decides its fate); a non-positive one is not.
            _keep = (
                ~_under
                & shapely.is_valid(b_poly)
                & (np.isnan(b_height) | (b_height > 0.0))
            )
            b_poly, b_height = b_poly[_keep], b_height[_keep]
            shapely.prepare(b_poly)
            b_bounds = shapely.bounds(b_poly)

            _known = np.isfinite(b_height)
            _area_m2 = (
                shapely.area(b_poly)
                * (111_320.0**2)
                * float(np.cos(np.radians((bbox[1] + bbox[3]) / 2.0)))
            )
            print(
                f"Overture buildings: {len(b_poly):,} footprints from {len(_hits)} file(s) "
                f"· height known for {_known.sum():,} ({_known.mean():.0%}, "
                f"{b_measured:,} measured + {int(_from_floors.sum()):,} from floors) · "
                f"median {np.nanmedian(b_height):.1f} m, tallest "
                f"{np.nanmax(b_height) if _known.any() else 0:.0f} m"
            )
            print(
                f"  median footprint {np.median(_area_m2):.0f} m^2 "
                f"(~{np.sqrt(np.median(_area_m2)):.0f} m across)"
            )
            if _known.mean() < 1.0:
                print(
                    f"  {(~_known).sum():,} with no height at all: they are drawn at the "
                    f"Default height, or skipped if that is 0."
                )

    mo.stop(not get_started(), None)
    return b_bounds, b_height, b_poly
...
@app.cell
def _(HALO, T, box_mean, np, smooth, tile_h, z_min):
    # THE GROUND, SMOOTHED ON THE HALO AND LEFT UNCROPPED. This is the
    ...
    # NO CROP HERE, WHICH IS THE ONE STRUCTURAL CHANGE FROM `xsql-naip-tiles.py`. The
    # buildings cell needs the full haloed field for two separate reasons, and both are the
    # reason the halo exists at all: a footprint that straddles a tile edge has to be stamped
    # into the NEIGHBOUR's halo as well, or the smoothing and the gradient at that edge see
    # a wall on one side and bare ground on the other; and a footprint's base elevation is
    # sampled from the tile that owns its centroid, which may be a tile over. Cropping is now
    # the last thing that happens to the height field rather than the first.
    ground = []
    ground_ok = []
    for _h in tile_h:
        ...
        ground.append(np.divide(_vs, _ms, out=np.zeros_like(_vs), where=_ms > 0) - z_min)
        ...
        ground_ok.append(_ms > 0)

    print(f"ground: {len(ground)} tile(s) haloed at {T + 1 + 2 * HALO}^2 · smooth radius {_r}")
    return ground, ground_ok
...
def _(
    ...
    # FOOTPRINTS INTO THE HEIGHT FIELD, AND THEN THE CROPS.
    #
    # A ROOF IS FLAT AND THE GROUND UNDER IT IS NOT. Adding `height` to whatever the terrain
    # does inside a footprint gives a building with a sloping roof sunk into a pit on the
    # uphill side, which is wrong in the way that reads as a bug rather than as a
    # simplification. So each footprint gets ONE base: the ground at its centroid, taken from
    # the tile that owns that centroid, and every texel inside the polygon is raised to
    # `base + height` with a `maximum` so overlapping footprints keep the taller roof.
    #
    # ONE BASE PER BUILDING, NOT ONE PER TILE, and that is the tiling-correctness argument
    # rather than an aesthetic one. A footprint crossing a tile boundary is stamped into both
    # tiles; if each tile computed its own base the roof would step by the terrain difference
    # exactly along the seam. Sampling once, from the owning tile, makes both stamps identical
    # by construction, the same way the lattice coordinates are identical by construction.
    #
    # The stamp is per polygon over its OWN index window, not a full-lattice test: a
    # `contains_xy` over (T + 49)^2 points per building would be minutes, over ~30x30 points
    # it is microseconds and the prepared geometry does the rest.
    _wide = slice(HALO - 1, HALO + T + 2)  # (T + 3)^2, for the hillshade
    _tight = slice(HALO, HALO + T + 1)  # (T + 1)^2, the tile itself

    _fill = float(default_height.value)
    _scale = float(building_scale.value)
    if b_poly is None or len(b_poly) == 0 or _scale <= 0.0:
        _z_fields = ground
        _stamped = _dropped = _unresolved = 0
    else:
        _h = np.where(np.isfinite(b_height), b_height, _fill if _fill > 0 else np.nan)
        _live = np.isfinite(_h) & (_h > 0)
        _dropped = int((~_live).sum())

        # THE OWNING TILE of a centroid is arithmetic, not a search: the grid is a quadtree
        # over the degree grid, so tile (i, j) is floor(lon / tile_deg), floor(lat / tile_deg).
        _by_xy = {(t["ix"], t["iy"]): _k for _k, t in enumerate(tiles)}
        _cx = shapely.get_x(shapely.centroid(b_poly))
        _cy = shapely.get_y(shapely.centroid(b_poly))
        _ti = np.floor(_cx / tile_deg).astype("int64")
        _tj = np.floor(_cy / tile_deg).astype("int64")

        _base = np.full(len(b_poly), np.nan)
        for _b in np.flatnonzero(_live):
            _k = _by_xy.get((int(_ti[_b]), int(_tj[_b])))
            if _k is None:
                continue  # centroid outside the tile grid: nothing owns it, so skip it
            # lon_h[n] = (i * T + n - HALO) * step, so the nearest lattice index inverts.
            _i = int(round(_cx[_b] / step)) - int(_ti[_b]) * T + HALO
            _j = int(round(_cy[_b] / step)) - int(_tj[_b]) * T + HALO
            _n = T + 1 + 2 * HALO
            if 0 <= _i < _n and 0 <= _j < _n and ground_ok[_k][_j, _i]:
                _base[_b] = ground[_k][_j, _i]

        _roof = _base + _h * _scale
        _ok = np.isfinite(_roof)
        _dropped += int(_live.sum() - _ok.sum())

        _z_fields = []
        _stamped = 0
        _unresolved = 0
        _raised = 0  # texels the stamps actually moved, counted on the tight crop only
        for _t, _g in zip(tiles, ground):
            _lon, _lat = _t["lon_h"], _t["lat_h"]
            _z = _g.copy()
            # Only the footprints whose bbox meets this tile's HALOED extent, which is what
            # puts a boundary-crossing building into both neighbours.
            _cand = np.flatnonzero(
                _ok
                & (b_bounds[:, 0] <= _lon[-1])
                & (b_bounds[:, 2] >= _lon[0])
                & (b_bounds[:, 1] <= _lat[-1])
            ...
            for _b in _cand:
                _i0 = int(np.searchsorted(_lon, b_bounds[_b, 0], "left"))
                _i1 = int(np.searchsorted(_lon, b_bounds[_b, 2], "right"))
                _j0 = int(np.searchsorted(_lat, b_bounds[_b, 1], "left"))
                _j1 = int(np.searchsorted(_lat, b_bounds[_b, 3], "right"))
                if _i1 <= _i0 or _j1 <= _j0:
                    # THE FOOTPRINT FELL BETWEEN LATTICE SAMPLES. Not an error and not
                    # silently ignored: it is counted, and the note below turns it into the
                    # one sentence that matters, which is that Detail is too coarse for the
                    # buildings in this scene.
                    _unresolved += 1
                    continue
                _X, _Y = np.meshgrid(_lon[_i0:_i1], _lat[_j0:_j1])
                _in = shapely.contains_xy(b_poly[_b], _X, _Y)
                if not _in.any():
                    _unresolved += 1
                    continue
                _sub = _z[_j0:_j1, _i0:_i1]
                _z[_j0:_j1, _i0:_i1] = np.where(_in, np.maximum(_sub, _roof[_b]), _sub)
                _stamped += 1
            _raised += int(np.count_nonzero((_z - _g)[_tight, _tight] > 0.0))
            _z_fields.append(_z)

    height_m = [_z[_wide, _wide] for _z in _z_fields]
    height = [_z[_tight, _tight] for _z in _z_fields]
    surface_ok = [_m[_tight, _tight] for _m in ground_ok]

    if b_poly is not None and len(b_poly):
        _texels = max(len(tiles) * (T + 1) ** 2, 1)
        print(
            f"buildings: {_stamped:,} footprint stamps over {len(tiles)} tile(s) "
            f"(a footprint on a tile edge is stamped into both) · {_unresolved:,} too small "
            f"for the lattice · {_dropped:,} with no usable height"
        )
        print(
            f"  raised {_raised:,} of {_texels:,} texels ({_raised / _texels:.2%} of the "
            f"surface is roof)"
        )
        if _unresolved > _stamped * 0.2:
            print(
                "  NOTE: more footprints fall between lattice samples than land on it. "
                "Lower Detail (m/texel) to resolve them, and remember the MESH samples the "
                "height field every Texels/quad texels, so Texels/quad = 1 is what puts a "
                "house on the geometry rather than only in the shading."
            )
    ...
    #
    # IT NOW CHECKS THE BUILDINGS TOO, for free, and this is the assertion that the
    # one-base-per-building rule actually holds: a footprint crossing a seam is stamped in
    # both tiles, from the same base and the same height, at lattice coordinates that are
    # the same float64, so the two columns of roof still have to agree to float32 noise. If
    # the base were ever sampled per tile, this number would jump to metres.
    ...
    print(
        ...
        f"seam {_worst:.3g} m over {_pairs} shared edge(s)"
    )
    if _worst > 0.01:
        print(
...
def _(T, bbox, m_texel_ew, m_texel_ns, mo, n_tiles, np, texels_per_quad):
    ...
    MAX_POS_MB = 512.0
    # THE CAP IS NOT WHERE IT BREAKS, WHICH IS WHY THIS SOFT NUMBER EXISTS. The 512 MB gate
    # is a refusal with a message; what actually happens well below it is that deck fails
    # to allocate the buffers and DRAWS NOTHING, with the basemap still rendering, the
    # camera still working, no Python error and nothing in the console except an unrelated
    # BitmapLayer assertion from the picker. That is a horrible thing to debug from the
    # symptom, so the number is printed as a warning before you get there. MEASURED: 12 MB
    # over 15 tiles draws; 606 MB over 192 tiles does not.
    SOFT_POS_MB = 128.0
    ...
    if SOFT_POS_MB < pos_mb <= MAX_POS_MB:
        print(
            f"  NOTE: {pos_mb:.0f} MB of positions ({n_tiles * len(triangles) / 1e6:.1f}M "
            f"triangles) is past the {SOFT_POS_MB:.0f} MB point where deck has been seen to "
            f"give up SILENTLY: basemap fine, camera fine, no mesh and no error. If the "
            f"scene comes back empty, this is the number to move. Raise Texels / quad "
            f"(each step quarters it) or coarsen Detail."
        )
    mo.stop(
        pos_mb > MAX_POS_MB,
        mo.md(
...
def _(PALETTES, mo):
    ...
    texels_per_quad = mo.ui.dropdown(
        ...
        # 2 BY DEFAULT HERE, NOT 4. This is the number that decides whether the MESH sees
        # the buildings the height field carries: at 4 texels per quad a 2 m lattice puts a
        # vertex every 8 m and a house is a bump between two of them. 1 is better geometry
        # and costs 4x the positions, which over a town-sized box is the difference between
        # 50 MB and 199 MB. Drop to 1 on a small box; that is what it is for.
        value="2",
        label="Texels / quad",
    )
...
def _(
    ...
    _buildings = mo.md(
        (
            f"<small>**{len(b_poly):,} Overture footprints** are part of this surface, "
            f"raised to a flat roof at ground + height. They take the same colour ramp and "
            f"the same hillshade as the terrain, because as far as everything downstream is "
            f"concerned they *are* terrain.</small>"
        )
        if b_poly is not None and len(b_poly)
        else "<small>No buildings in this surface.</small>"
    )
    if view == "NDVI":
        _body = mo.md(
        ...
        _out = mo.vstack([_strip, _body, _buildings, _scheme], gap=0.25)
    elif view == "NAIP RGB":
        _out = mo.vstack(
            [
                mo.md(
