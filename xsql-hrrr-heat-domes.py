...
# ]
# ///
"""HRRR heat, per H3 res 6 cell over CONUS, as a film with a memory, WITH THE
BOUNDARIES OF THE SUSTAINED HEAT DRAWN AND MOVING (2026-08-18), on the xarray-sql
0.4.0 pre-release (multi-engine backends).

WHAT IS NEW OVER xsql-hrrr-heat-hex.py. (1) The fold has an ENGINE switch: the same
fold SQL runs on DataFusion (XarrayContext, the h3ronpy UDF, the default) or on DuckDB
through the rc's `xql.register(con, "cube", ds)` (its pushdown pyarrow dataset) with
duckdb-h3's `h3_latlng_to_cell` in the GROUP BY. Measured on the real cube, young
chunk, 159 h, 2 variables: DataFusion 39 s, DuckDB 179 s, identical 33.5M rows and
means (docs/xarray-sql-multi-backend-notes.md). (2) The BOUNDARIES: at each contour
level (CONTOURS, degC of sustained excess) the browser draws the edge of the set of
cells at or above it, every frame, following the clock and the sliders. It is H3's
cellsToMultiPolygon before the ring-linking walk: a cell's directed edge is on the
boundary when its destination is not in the set; a per-cell 6-neighbour index is
built once in the browser (h3-js), then a frame is one pass over the load matrix and
a PathLayer of the surviving edges. Zero bytes across the bridge, live with the
sliders. (3) A DOME TABLE from DuckDB, kernel-side, on the same fold output: numpy
runs the accumulator at the kernel's slider defaults, DuckDB dissolves each
(frame, level) set with h3_cells_to_multi_polygon_wkb (2 s for a week) and ST_Dumps
it into blobs with area (Albers) and centroid; blobs under DOME_MIN_KM2 (speckle,
median blob = one cell) are dropped, and the largest per frame at each level is the
dome's track. That is where DuckDB earns its place: identity and numbers, not the
outline.
...
and give a clicked cell its county name; the dome dissolve), DataFusion does the fold
by default (DuckDB through the rc as the switch), numpy the heat index. con.register throughout (marimo mangles underscore locals, so DuckDB's
...
def _(mo):
    mo.md(r"""
    [![Open in molab](https://molab.marimo.io/molab-shield.svg)](https://molab.marimo.io/github/github.com/kentstephen/x-sql-marimo/blob/main/xsql-hrrr-heat-domes.py)
    ...
    **Run this one in molab if you can.** The wait here is bandwidth: every window
    ...
    # HRRR heat with a memory, and its boundaries
    ...
    it; press play. New here: the **boundaries** of the sustained heat, drawn at
    fixed levels (1, 3, 5, 10 °C of sustained excess) as lines that move with the
    film and follow the sliders; and a table below the map, computed by DuckDB,
    of the domes as objects (area, centre, track).
    ...
    **What the notebook does with them.** The store is queried with
    [xarray-sql](https://github.com/xqlsystems/xarray-sql) 0.4.0 (pre-release), which
    registers the Zarr cube as a table on either engine, DataFusion or DuckDB
    (`ENGINE` in the constants cell; same SQL), and prunes the store columns that
    never touch CONUS land.
...
def _():
    ...
    RES = 6
    # THE ENGINE, the rc's seam: "datafusion" registers the cube on an XarrayContext
    # (h3ronpy UDF in the GROUP BY); "duckdb" registers it on the DuckDB connection
    # with xql.register (pushdown dataset) and folds with duckdb-h3's
...
def _(anywidget, traitlets):
    class HexFilm(anywidget.AnyWidget):
        ...
        import {latLngToCell, originToDirectedEdges, getDirectedEdgeDestination, directedEdgeToBoundary} from "https://esm.sh/h3-js@4.5.0";
        ...
        function render({model, el}) {
          ...
          root.innerHTML = `<style>${CSS}</style>
            <div class="hf-map">
              <div class="hf-hud hf-tl"><div class="hf-card hf-panel">
                ...
                <div class="hf-fields"><button class="hf-b hf-fi hf-on" data-field="index" title="NWS heat index this hour (I)">heat index</button><button class="hf-b hf-fi" data-field="load" title="sustained heat: exponentially weighted mean of heat index above the threshold (L)">sustained heat</button><button class="hf-b hf-bnd hf-on" title="boundaries of the sustained heat at the contour levels, on either field (B)">boundaries</button></div>
                <div class="hf-dim hf-bndnote"></div>
                ...
                <div class="hf-body">
                  ...
                  <div class="hf-dim hf-hint">click a cell for its value and line · space plays · ← → step · I / L switch field · B boundaries</div>
                </div>
              </div></div>
          ...
          // BOUNDARIES: per-cell 6-neighbour index (row into hexes, -1 = not a land cell),
          // built once from h3-js; edge coords cached lazily by (cell*6+k). A frame's
          ...
          function buildNbr() {
            ...
            nbr = new Int32Array(N * 6).fill(-1);
            for (let i = 0; i < N; i++) {
              const es = originToDirectedEdges(hexes[i]);
              for (let k = 0; k < es.length && k < 6; k++) nbr[i * 6 + k] = hexIndex.get(getDirectedEdgeDestination(es[k])) ?? -1;
            }
            nbrMs = Math.round(performance.now() - t0);
          }
          let nbrMs = 0;
          function edgeCoords(i, k) {
            const key = i * 6 + k; let c = edgeXY.get(key);
            if (!c) {
              const b = directedEdgeToBoundary(originToDirectedEdges(hexes[i])[k]);  // [[lat,lng],[lat,lng]]
              c = [b[0][1], b[0][0], b[1][1], b[1][0]]; edgeXY.set(key, c);
            }
            return c;
          }
          function boundaryLayers() {
            if (!showBounds || !load || !N || !F) return [];
            buildNbr();
            const levels = (cfg.contours && cfg.contours.length) ? cfg.contours : [1, 3, 5, 10];
            const out = [], base = frame * N;
            levels.forEach((lvl, ci) => {
              const lq = Math.round(lvl * 10), pos = [];
              for (let i = 0; i < N; i++) {
                const v = load[base + i]; if (v === 255 || v < lq) continue;
                for (let k = 0; k < 6; k++) {
                  const j = nbr[i * 6 + k];
                  if (j >= 0) { const w = load[base + j]; if (w !== 255 && w >= lq) continue; }
                  const c = edgeCoords(i, k); pos.push(c[0], c[1], c[2], c[3]);
                }
              }
              const E = pos.length / 4; if (!E) return;
              const startIndices = new Uint32Array(E + 1); for (let e = 0; e <= E; e++) startIndices[e] = 2 * e;
              out.push(new PathLayer({
                id: "bound-" + lvl,
                data: {length: E, startIndices, attributes: {getPath: {value: new Float32Array(pos), size: 2}}},
                _pathType: "open",
                getColor: [BND_RGB[0], BND_RGB[1], BND_RGB[2], 150 + Math.min(105, ci * 35)],
                getWidth: 1 + ci * 0.6, widthUnits: "pixels", widthMinPixels: 1,
          ...
          const bndBtn = q(".hf-bnd"), bndNote = q(".hf-bndnote");
          function bndLabel() {
            const levels = (cfg.contours && cfg.contours.length) ? cfg.contours : [1, 3, 5, 10];
            bndNote.textContent = showBounds ? `boundaries: sustained heat ≥ ${levels.map(v => v + "°C").join(" / ")} (thin to thick), following the sliders` : "";
          }
          bndBtn.onclick = () => { showBounds = !showBounds; bndBtn.classList.toggle("hf-on", showBounds); bndLabel(); update(); };
          ...
          const rulerText = () => `${N.toLocaleString()} cells · ${F} frames` + (nbrMs ? ` · neighbours ${nbrMs} ms` : "");
          function boot() {
          ...
          model.on("change:cells", () => { nbr = null; edgeXY = new Map(); loadCells(); loadFrames(); ruler.textContent = rulerText(); update(); });
...
def _(
    ...
    _key = (ENGINE, int(RES), int(pix2h.num_rows), tuple(VARS), str(t0), str(t1))
    if HOLD["key"] == _key and HOLD["cell_hour"] is not None:
        ...
        _fold_sql = f"""
            SELECT t, h3_latlng_to_cell(lat, lon, {{res}}) AS hex, {", ".join(_cols)}
            FROM cube JOIN pix2h USING (y, x)
            WHERE temperature_2m = temperature_2m AND ({land_pred})
            GROUP BY 1, 2
        """
        if ENGINE == "duckdb":
            # THE RC'S SEAM: the same lazy cube registered on the DuckDB connection as
            # a pushdown pyarrow dataset (projection pushdown, chunk pruning on the
            # block predicate, prefetch pool), the same SQL, duckdb-h3's UDF. The
            # DataFusion context above is built and unused on this branch; the pool
            # knobs do not apply (DuckDB spills on its own).
            con.register("pix2h", pix2h)
            xql.register(con, "cube", _cube, chunks={"t": _hours, "y": 45, "x": 45})
            cell_hour = con.sql(_fold_sql.format(res=int(RES))).arrow().read_all()
            con.unregister("cube")
            con.unregister("pix2h")
        else:
            cell_hour = ctx.sql(_fold_sql.format(res=f"CAST({int(RES)} AS INT)")).to_arrow_table()
        fold_stats = (
...
def _(mo):
    ...
    the kernel's: numpy runs the same accumulator at the constants-cell defaults,
    DuckDB dissolves the set of cells at or above each level for every hour with
    `h3_cells_to_multi_polygon_wkb` (H3's own outer-boundary walk; the `ST_Union_Agg`
    of hexagons is 30x slower on the same input) and `ST_Dump`s the multipolygons
    into blobs with an area (Albers) and a centroid. Blobs under `DOME_MIN_KM2` are
    speckle at the band edge (the median blob is a single cell) and are dropped; the
    largest blob per hour at each level is the dome and its centroid over the hours
    is its track. About two seconds for a week.
    """)
    return
...
def _(CONTOURS, DOME_MIN_KM2, HALF_LIFE, RAIN_FLUSH, THRESHOLD, WIND_VENT, cells, con, frame_labels, has_rain, has_wind, hi_q, mo, np, pa, wx_q):
    ...
    _lvl = np.zeros_like(_L, dtype=np.int16)
    for _c in CONTOURS:
        _lvl[_L >= _c] = int(round(_c * 10))
    _fi, _ci = np.nonzero(_lvl > 0)
    con.register("dome_mask", pa.table({"f": _fi.astype(np.int32), "cell": cells[_ci], "lvl": _lvl[_fi, _ci]}))
    con.register("dome_lvl", pa.table({"lvl": np.array([int(round(c * 10)) for c in CONTOURS], dtype=np.int16)}))
    # cumulative sets (a cell at 5 is also in the 1 and 3 sets), dissolved by H3, dumped, measured
    domes = con.sql(f"""
        WITH u AS (
    ...
    """).arrow().read_all()
    # the dome per hour at the second level (or the first if only one): the track
    ...
    dome_track = con.sql(f"""
        ...
        FROM (SELECT *, row_number() OVER (PARTITION BY f ORDER BY km2 DESC) AS rn FROM domes WHERE level = {_track_level})
        WHERE rn = 1 ORDER BY f
    """).arrow().read_all()
    con.unregister("domes")
    _rows = [
        {"level °C": r["level"], "blobs ≥ %.0f km²" % DOME_MIN_KM2: r["blobs"], "largest km²": f"{r['max_km2']:,.0f}",
         "at (UTC)": frame_labels[int(r["peak_frame"])], "centre lat, lon": f"{r['peak_lat']}, {r['peak_lon']}"}
        for r in dome_summary.to_pylist()
    ]
    _trk = [
        {"hour (UTC)": frame_labels[int(r["f"])], "km²": f"{r['km2']:,.0f}", "centre lat": r["lat"], "centre lon": r["lon"]}
        for r in dome_track.to_pylist()
    ]
    dome_stats = (
        f"domes: {len(_fi):,} cell-hours over {CONTOURS[0]:g} °C · {domes.num_rows:,} blobs ≥ {DOME_MIN_KM2:g} km² · "
        f"dissolve + dump {_dtime.perf_counter() - _dt0:.1f}s on DuckDB"
    )
    mo.vstack([
